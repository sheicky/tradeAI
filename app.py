...
try:
    ...
    st.error(
        "Model file not found. Please make sure 'models/tunned_rf_model.pkl' exists."
    )
...
def make_predictions(input_df):
    try:
        ...
        probabilities = {"Random Forest": probability}
        ...
        with col1:
            st.metric("Signal", signal["action"], delta=signal["confidence"])
        with col2:
            st.metric(
                "Position Size",
                f"${signal['size']:,.0f}",
        ...
        with context_col2:
            st.metric(
                "Interest Rate", signal["market_context"]["interest_rate_environment"]
            )
        with context_col3:
...
with col1:
    XAU_BGNL = st.number_input(
        "🏆 Gold Price (XAU BGNL)", value=1800.0, help="Current gold price in USD"
    )
    VIX = st.number_input(
        "📊 Volatility Index (VIX)", value=20.0, help="Market fear gauge"
    )
    LUMSTRUU = st.number_input(
        "🌍 Global Bond Index", value=100.0, help="Global bond market performance"
    )
    LMBITR = st.number_input(
        "📈 Market Index", value=100.0, help="Overall market performance"
    )

with col2:
    LUACTRUU = st.number_input(
        "💰 Aggregate Bond Index", value=100.0, help="US bond market indicator"
    )
    LUAGTRUU = st.number_input(
        "📋 Treasury Index", value=100.0, help="Treasury market performance"
    )
    USGG10YR = st.number_input(
        "🏛️ US 10Y Treasury Yield", value=2.0, help="Benchmark interest rate"
    )
...
if st.button("🔍 Analyze Market"):
    with st.spinner("Analyzing market conditions..."):
        ...
        if result is not None:
            ...
            # Graphique de jauge pour la probabilité moyenne
            ...
            with signal_col1:
                st.metric("Action", signal["action"], delta=signal["confidence"])
            with signal_col2:
                st.metric(
                    "Position Size",
                    f"${signal['size']:,.0f}",
...
with chat_col:
    ...
    user_input = st.text_input(
        "Ask about market strategy, risks, or portfolio:", key="user_input"
    )
    ...
    if user_input:
        with st.spinner("Analyzing your question..."):
            ...
            context = {
                ...
                "recent_performance": (
                    sum(strategy.returns[-30:]) if strategy.returns else 0
                ),
            }
